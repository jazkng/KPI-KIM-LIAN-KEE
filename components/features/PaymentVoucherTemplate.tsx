// ============================================================
// PaymentVoucherTemplate.tsx  (v5 — Multi-page Support)
...
export const PaymentVoucherTemplate = forwardRef<HTMLDivElement, PaymentVoucherTemplateProps>(
    ({ voucher }, ref) => {
        ...
        // 🌟 v5: Safe pagination to handle long list of items without cutting off signature blocks
        ...
        return (
            <div
                ref={ref}
                id="payment-voucher-export-root-wrapper"
                className="flex flex-col gap-8 bg-gray-100"
                style={{ width: '794px' }}
            >
                {Array.from({ length: totalPages }).map((_, pageIdx) => {
                    const startIdx = pageIdx * ITEMS_PER_PAGE;
                    const pageRows = rows.slice(startIdx, startIdx + ITEMS_PER_PAGE);
                    const padCount = Math.max(0, ITEMS_PER_PAGE - pageRows.length);
                    const isLastPage = pageIdx === totalPages - 1;

                    return (
                        <div
                            key={pageIdx}
                            id={`payment-voucher-export-page-${pageIdx}`}
                            className={`bg-white ${isA5 ? 'px-8 py-3' : 'p-12'} text-black font-sans relative border-4 border-double border-gray-300 antialiased flex flex-col justify-between`}
                            style={{
                                width: '794px',
                                minWidth: '794px',
                                maxWidth: '794px',
                        ...
                        >
                                    ...
                                    <div className={`border ${isA5 ? 'p-1.5' : 'p-4'} rounded`}>
                                        ...
                                        <div className="flex gap-4 items-center mt-1">
                                            <div className="flex items-center gap-1.5">
                                                <span 
                                                    className={`border-2 border-black shrink-0 ${isCash ? 'bg-black' : 'bg-white'}`}
                                                    style={{
                                                        width: isA5 ? '14px' : '20px',
                                                        height: isA5 ? '14px' : '20px',
                                                        minWidth: isA5 ? '14px' : '20px',
                                                        minHeight: isA5 ? '14px' : '20px',
                                                        maxWidth: isA5 ? '14px' : '20px',
                                                        maxHeight: isA5 ? '14px' : '20px',
                                                        display: 'inline-flex',
                                                        alignItems: 'center',
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                <span 
                                                    className={`border-2 border-black shrink-0 ${isOnline ? 'bg-black' : 'bg-white'}`}
                                                    style={{
                                                        width: isA5 ? '14px' : '20px',
                                                        height: isA5 ? '14px' : '20px',
                                                        minWidth: isA5 ? '14px' : '20px',
                                        ...
                                        {pageRows.map((p, i) => {
                                            ...
                                            return (
                                                <tr key={i} className="border-t border-gray-300">
                                                    <td className={`${isA5 ? 'p-1 text-xs' : 'p-3 text-sm'} border-r border-black text-center font-mono align-top`}>{globalIdx + 1}</td>
                            ...
                            <div className="mt-auto">
                                {/* === Signatures === */}
                                <div className={`grid grid-cols-3 gap-8 ${isA5 ? 'mt-1' : 'mt-6'}`}>
                                    <div className="text-center">
                                        <div className={`border-b border-black mb-1 ${isA5 ? 'h-8' : 'h-12'} flex items-end justify-center pb-0.5`}>
                                ...
                                <div className={`flex justify-between items-center ${isA5 ? 'mt-1.5' : 'mt-4'}`}>
                                    <p className="text-[8px] text-gray-400 text-left leading-normal">
