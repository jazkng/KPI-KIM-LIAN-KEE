...
import { getBaseUnit, getPreferredCountQuantity } from '../../../utils/unitConversion';
...
export const TaskEditorModal: React.FC<TaskEditorModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-0 md:p-4 backdrop-blur-sm animate-in fade-in overscroll-contain">
            <div className="bg-white w-full h-full md:max-w-lg md:h-auto md:max-h-[90vh] md:rounded-3xl shadow-2xl animate-in zoom-in-95 flex flex-col relative overflow-hidden">
                <div className="p-4 pt-[max(1rem,env(safe-area-inset-top))] md:p-6 md:pb-3 shrink-0 border-b border-gray-100 bg-white z-10">
                    ...
                    <div className="flex gap-1.5 mb-3 overflow-x-auto pb-1">
                        {FREQ_OPTIONS.map(opt => (
                            <button key={opt.value} onClick={() => onFrequencyChange(opt.value)}
                                className={`px-3 min-h-[44px] rounded-lg text-[10px] font-bold whitespace-nowrap border transition-all ${editTaskFrequency === opt.value ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-500'}`}>
                                {opt.label}
                            </button>
                        ))}
                    </div>
                    <div className="relative">
                        ...
                        <input type="search" inputMode="search" placeholder="搜索物品..." value={editTaskSearchTerm} onChange={(e) => onSearchChange(e.target.value)}
                            className="w-full min-h-[44px] pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold outline-none focus:border-indigo-400"/>
                    </div>
                </div>
                ...
                <div className="p-4 overflow-y-auto flex-grow space-y-2 bg-[#F9FAFB]">
                    {editTaskStockData
                        ...
                        .map(stockItem => {
                            ...
                            const countQty = getPreferredCountQuantity(stockItem);
                            const countUnit = stockItem.preferredStockViewUnit === 'DISPLAY' && stockItem.displayUnit
                                ? stockItem.displayUnit
                                : getBaseUnit(stockItem);
                            return (
                ...
                <div className="p-4 pb-[max(1rem,env(safe-area-inset-bottom))] border-t border-gray-100 bg-white flex gap-3 shrink-0">
                    <button onClick={onClose} className="flex-1 min-h-[52px] py-3 bg-white border-2 border-gray-200 text-gray-600 font-bold rounded-xl hover:bg-gray-50">取消</button>
                    <button onClick={onSave}
                        className="flex-[2] min-h-[52px] py-3 bg-indigo-600 text-white font-black rounded-xl shadow-lg hover:bg-indigo-700 flex items-center justify-center gap-2">
